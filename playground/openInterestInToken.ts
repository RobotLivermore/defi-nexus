...
const main = async () => {
  ...
  logReader.forEach(
    (log) => {
      if (log.eventName === 'OpenInterestInTokensUpdated') {
        const key = `${log.market.toLowerCase()}-${log.isLong ? 'long' : 'short'}`
        if (!marketTokenValueMap[key]) {
          marketTokenValueMap[key] = []
        }
        if (log.poolValue === '0' || log.marketTokensSupply === '0') {
          return
        }
        marketTokenValueMap[key].push({
  ...
  fs.writeFileSync(
    './data/analysis/openInterestInTokenUpdate.json',
